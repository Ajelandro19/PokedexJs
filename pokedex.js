...
const fetchPokemon = () => {
    ...
    fetch(url).then((res) => {
        if (res.status != "200") {
            ...
            pokeImgInput.style.top="15px";
            pokeImgInput.style.left="30px";
            fondo.style.background =  `radial-gradient(black 33%, black 33%)`;
        }
        else {
            return res.json();
        }
        ...
        if (data) {
            ...
            pokeImgInput.style.top=null;
            pokeImgInput.style.left=null;
        }
    });
}
const typeColors = {
